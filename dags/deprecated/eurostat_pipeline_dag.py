...
with DAG(
    ...
    task_identify_health_datasets = BashOperator(
        ...
        bash_command=f"mkdir -p /opt/airflow/staging && python {SCRIPTS_PATH}/SourceData.py --action export-details --keyword HLTH --csv-output /opt/airflow/staging/health_datasets_details.csv",
        ...
        doc_md="Filters catalog for 'HLTH' datasets and exports their details to a CSV in /opt/airflow/staging/."
    )
    ...
    task_load_to_postgres = BashOperator(
        ...
        env={ # TODO: Replace with Airflow Connection for PostgreSQL
